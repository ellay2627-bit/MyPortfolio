...
const RichTextEditor = ({ value, onChange }: { value: string; onChange: (val: string) => void }) => {
  ...
  if (!mounted) {
    return <textarea value={value} onChange={(e) => onChange(e.target.value)} className="w-full h-40 px-4 py-2 bg-gray-700 rounded-lg text-white" />;
  }

  return (
    <Suspense fallback={<textarea value={value} onChange={(e) => onChange(e.target.value)} className="w-full h-40 px-4 py-2 bg-gray-700 rounded-lg text-white" />}>
      <ReactQuill
        value={value}
        onChange={onChange}
