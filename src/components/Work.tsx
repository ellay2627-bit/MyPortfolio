        ...
        >
          ...
          {!imageLoaded && (
            <div className="absolute inset-0 z-5">
              <div className="absolute inset-0 bg-white/[0.04] animate-shimmer"></div>
            </div>
          )}
...
export default function Work({ directWorkId, onDirectClose }: WorkProps = {}) {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting) {
          // 当哨兵元素进入视口时，加载更多
          if (!loading && filteredWorks.length > currentPage * itemsPerPage) {
  ...
  }, [loading, currentPage, itemsPerPage, filteredWorks.length]);
  ...
  return (
    <section id="work" className="pt-48 pb-80 md:pt-32 md:pb-60 bg-dark-bg relative overflow-x-hidden">
      ...
      <div className="w-full px-4 md:container md:mx-auto md:px-4 relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 lg:gap-16 mt-2 sm:mt-12 md:mt-16 lg:mt-24 w-full">
          {loading ? (
            ...
            // 显示作品卡片
            filteredWorks.slice(0, currentPage * itemsPerPage).map((work, index) => (
              <WorkCard 
                key={work.id}
                work={work} 
                index={index} 
      ...
      {/* 无限滚动哨兵元素 */}
      {!loading && filteredWorks.length > currentPage * itemsPerPage && (
