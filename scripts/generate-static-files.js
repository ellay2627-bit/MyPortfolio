...
// 处理单个作品
function processWork(work, workId) {
  // 处理封面
  if (work.cover) {
    // 检测文件类型
    let ext = 'jpg';
    if (work.cover.includes('png')) ext = 'png';
    else if (work.cover.includes('gif')) ext = 'gif';
    else if (work.cover.includes('webp')) ext = 'webp';
    
    const coverFileName = `work-${workId}-cover.${ext}`;
  ...
  if (work.media && Array.isArray(work.media)) {
    work.media.forEach((mediaItem, index) => {
      if (mediaItem.url) {
        // 检测文件类型
        let ext = 'jpg';
        if (mediaItem.url.includes('png')) ext = 'png';
        else if (mediaItem.url.includes('gif')) ext = 'gif';
        else if (mediaItem.url.includes('webp')) ext = 'webp';
        
        const mediaFileName = `work-${workId}-media-${index}.${ext}`;
...
function main() {
  ...
  for (const file of files) {
    ...
    try {
      ...
      // 处理作品
      const processedWork = processWork(work, workId);
      works.push(processedWork);
      
      // 同时也更新原来的单个作品文件（确保没有 base64）
      fs.writeFileSync(filePath, JSON.stringify(processedWork, null, 2));
      
      console.log(`处理完成: ${work.title || workId}`);
    } catch (error) {
  ...
  console.log('\n=== 完成 ===');
  console.log(`\ndata.json 大小: ${Math.round(fs.statSync(dataJsonPath).size / 1024)} KB`);
}
